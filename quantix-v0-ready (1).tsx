...
const styles = `
  ...
  /* v0-safe font fallbacks if above is blocked */
    ...
    <div className="auth-page">
      <div className="auth-card">
        ...
        <div className="form-group">
          ...
          <input id="login-email" name="email" className="form-input" type="email" placeholder="admin@quantix.io" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} autoComplete="email" />
        </div>
        <div className="form-group">
          <label className="form-label" htmlFor="login-password">Password</label>
          <input id="login-password" name="password" className="form-input" type="password" placeholder="••••••••" value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} autoComplete="current-password" />
        </div>
      ...
      <div className="auth-card">
        ...
        {["name","email","password","confirm"].map(f => (
          <div className="form-group" key={f}>
            <label className="form-label" htmlFor={`reg-${f}`}>{f==="confirm"?"Confirm password":f.charAt(0).toUpperCase()+f.slice(1)}</label>
            <input id={`reg-${f}`} name={f==="confirm"?"confirmPassword":f} className="form-input" type={f.includes("pass")||f==="confirm"?"password":"text"} placeholder={f==="name"?"Alex Morgan":f==="email"?"you@company.com":"••••••••"} value={form[f]} onChange={e=>setForm(p=>({...p,[f]:e.target.value}))} autoComplete={f==="name"?"name":f==="email"?"email":f==="password"?"new-password":"new-password"} />
        ...
        {sent ? (
          ...
          <>
            <div className="form-group">
              ...
              <input id="forgot-email" name="email" className="form-input" type="email" placeholder="admin@quantix.io" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email" />
            </div>
  ...
  return (
    <div className="header">
      ...
      <div className="h-search">
        ...
        <input id="global-search" name="q" placeholder="Search anything…" autoComplete="off" />
      </div>
      <button className="h-btn" onClick={toggleDark} title="Toggle theme">
        {darkMode
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              {reportSaved ? (
                ...
                <>
                  <div className="form-group">
                    <label className="form-label" htmlFor="report-name">Report Name *</label>
                    <input id="report-name" name="reportName" className="form-input" placeholder="e.g. Q3 Revenue Summary" value={reportForm.name} onChange={e=>setReportForm(p=>({...p,name:e.target.value}))} autoComplete="off" />
                  ...
                  <div className="form-group">
                    ...
                    <select id="report-type" name="reportType" className="form-input form-select" value={reportForm.type} onChange={e=>setReportForm(p=>({...p,type:e.target.value}))}>
                  ...
                  <div className="form-group">
                    ...
                    <select id="report-range" name="reportRange" className="form-input form-select" value={reportForm.range} onChange={e=>setReportForm(p=>({...p,range:e.target.value}))}>
    ...
    <div>
      ...
      <div className="card">
        <div className="flex gap-3 mb-3" style={{flexWrap:"wrap"}}>
          <input id="crm-search" name="search" className="form-input" style={{maxWidth:240}} placeholder="🔍 Search leads…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} autoComplete="off" />
          <div className="toggle-group">
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              {[["name","Full Name","text","Alex Morgan"],["company","Company","text","Acme Corp"],["email","Email","email","alex@acme.com"],["value","Deal Value","number","25000"]].map(([f,l,t,ph])=>(
                <div className="form-group" key={f}>
                  <label className="form-label" htmlFor={`crm-${f}`}>{l}</label>
                  <input id={`crm-${f}`} name={f} className="form-input" type={t} placeholder={ph} value={form[f]} onChange={e=>setForm(p=>({...p,[f]:e.target.value}))} autoComplete="off" />
                  {errors[f] && <div className="form-error">{errors[f]}</div>}
                </div>
              ))}
              <div className="form-group">
                <label className="form-label" htmlFor="crm-status">Status</label>
                <select id="crm-status" name="status" className="form-input form-select" value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group">
                <label className="form-label" htmlFor="inv-client">Client Name *</label>
                <input id="inv-client" name="client" className="form-input" placeholder="e.g. Acme Corp" value={invForm.client} onChange={e=>setInvForm(p=>({...p,client:e.target.value}))} autoComplete="off" />
                {invErrors.client && <div className="form-error">{invErrors.client}</div>}
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="inv-amount">Amount (USD) *</label>
                <input id="inv-amount" name="amount" className="form-input" type="number" placeholder="e.g. 5000" value={invForm.amount} onChange={e=>setInvForm(p=>({...p,amount:e.target.value}))} autoComplete="off" />
                {invErrors.amount && <div className="form-error">{invErrors.amount}</div>}
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="inv-status">Status</label>
                <select id="inv-status" name="status" className="form-input form-select" value={invForm.status} onChange={e=>setInvForm(p=>({...p,status:e.target.value}))}>
                  {["Draft","Pending","Paid","Overdue"].map(s=><option key={s}>{s}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="inv-date">Invoice Date</label>
                <input id="inv-date" name="invoiceDate" className="form-input" type="date" value={invForm.date} onChange={e=>setInvForm(p=>({...p,date:e.target.value}))} />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={()=>setShowModal(false)}>Cancel</button>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input id="proj-name" name="projectName" className="form-input" placeholder="e.g. Platform v3 Launch" value={form.name} onChange={e=>setForm(prev=>({...prev,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addProject()} autoComplete="off" />
                {formError && <div className="form-error">{formError}</div>}
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="proj-priority">Priority</label>
                <select id="proj-priority" name="priority" className="form-input form-select" value={form.priority} onChange={e=>setForm(prev=>({...prev,priority:e.target.value}))}>
                  {["Low","Medium","High","Critical"].map(v=><option key={v}>{v}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="proj-due">Due Date</label>
                <input id="proj-due" name="dueDate" className="form-input" type="date" value={form.due} onChange={e=>setForm(prev=>({...prev,due:e.target.value}))} />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="proj-team">Team Members <span style={{color:"var(--text3)",fontWeight:400}}>(comma-separated initials, e.g. AK, SR)</span></label>
                <input id="proj-team" name="team" className="form-input" placeholder="AK, SR, MJ" value={form.team} onChange={e=>setForm(prev=>({...prev,team:e.target.value}))} autoComplete="off" />
              </div>
            </div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input id="task-title" name="title" className="form-input" placeholder="e.g. Implement auth flow" value={taskForm.title} onChange={e=>setTaskForm(prev=>({...prev,title:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addTask()} autoFocus autoComplete="off" />
                {taskError && <div className="form-error">{taskError}</div>}
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="task-column">Column</label>
                <select id="task-column" name="column" className="form-input form-select" value={taskForm.column} onChange={e=>setTaskForm(prev=>({...prev,column:e.target.value}))}>
              ...
              <div className="grid-2">
                <div className="form-group">
                  ...
                  <select id="task-tag" name="tag" className="form-input form-select" value={taskForm.tag} onChange={e=>setTaskForm(prev=>({...prev,tag:e.target.value}))}>
                    {["Dev","Design","Research","Testing","DevOps","Docs","QA","Marketing"].map(t=><option key={t}>{t}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="form-label" htmlFor="task-priority">Priority</label>
                  <select id="task-priority" name="priority" className="form-input form-select" value={taskForm.priority} onChange={e=>setTaskForm(prev=>({...prev,priority:e.target.value}))}>
                    {["Low","Medium","High","Critical"].map(v=><option key={v}>{v}</option>)}
                  </select>
                </div>
              </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label" htmlFor="event-date">Date</label><input id="event-date" name="eventDate" className="form-input" type="date" value={selDate} onChange={e=>setSelDate(e.target.value)} /></div>
              <div className="form-group"><label className="form-label" htmlFor="event-title">Event Title</label><input id="event-title" name="eventTitle" className="form-input" placeholder="e.g. Team standup" value={newTitle} onChange={e=>setNewTitle(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addEvent()} autoComplete="off" /></div>
            </div>
          ...
          <div className="chat-main">
            ...
            <div className="chat-input-area">
              <div className="chat-input-row">
                <textarea id="chat-input" name="message" className="chat-input" placeholder="Type a message… (Enter to send)" value={input}
      ...
      {showCompose && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowCompose(false)}>
          <div className="modal modal-lg">
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label" htmlFor="compose-to">To</label><input id="compose-to" name="to" className="form-input" type="email" placeholder="recipient@example.com" value={compose.to} onChange={e=>setCompose(p=>({...p,to:e.target.value}))} autoComplete="email" />{composeErrors.to&&<div className="form-error">{composeErrors.to}</div>}</div>
              <div className="form-group"><label className="form-label" htmlFor="compose-subject">Subject</label><input id="compose-subject" name="subject" className="form-input" placeholder="Email subject" value={compose.subject} onChange={e=>setCompose(p=>({...p,subject:e.target.value}))} autoComplete="off" />{composeErrors.subject&&<div className="form-error">{composeErrors.subject}</div>}</div>
              <div className="form-group"><label className="form-label" htmlFor="compose-body">Message</label><textarea id="compose-body" name="body" className="form-input" rows={8} placeholder="Write your message…" value={compose.body} onChange={e=>setCompose(p=>({...p,body:e.target.value}))} /></div>
            </div>
            <div className="modal-footer">
        ...
        <div className="grid-2" style={{alignItems:"start"}}>
          <div className="card">
            ...
            <div className="flex items-center gap-4 mb-4">
              <label style={{cursor:"pointer"}}>
                ...
                <input id="avatar-upload" name="avatar" type="file" accept="image/*" style={{display:"none"}} onChange={handleAvatarChange} />
              </label>
              <div>
          ...
          <div className="card">
            ...
            {[["name","Full Name","text"],["email","Email","email"],["company","Company","text"],["role","Role","text"]].map(([f,l,t])=>(
              <div className="form-group" key={f}>
                <label className="form-label" htmlFor={`profile-${f}`}>{l}</label>
                <input id={`profile-${f}`} name={f} className="form-input" type={t} value={profile[f]} onChange={e=>setProfile(p=>({...p,[f]:e.target.value}))} autoComplete={f==="email"?"email":f==="name"?"name":"off"} />
                {errors[f]&&<div className="form-error">{errors[f]}</div>}
              </div>
            ))}
      ...
      {showAddCard && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowAddCard(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label" htmlFor="card-number">Card Number</label><input id="card-number" name="cardNumber" className="form-input" placeholder="1234 5678 9012 3456" maxLength={19} value={newCard.number} onChange={e=>setNewCard(p=>({...p,number:formatCardNum(e.target.value)}))} autoComplete="cc-number" inputMode="numeric" />{cardErrors.number&&<div className="form-error">{cardErrors.number}</div>}</div>
              <div className="form-group"><label className="form-label" htmlFor="card-name">Cardholder Name</label><input id="card-name" name="cardName" className="form-input" placeholder="Alex Morgan" value={newCard.name} onChange={e=>setNewCard(p=>({...p,name:e.target.value}))} autoComplete="cc-name" />{cardErrors.name&&<div className="form-error">{cardErrors.name}</div>}</div>
              <div className="grid-2">
                <div className="form-group"><label className="form-label" htmlFor="card-expiry">Expiry</label><input id="card-expiry" name="cardExpiry" className="form-input" placeholder="MM/YY" maxLength={5} value={newCard.expiry} onChange={e=>setNewCard(p=>({...p,expiry:e.target.value.replace(/[^\d\/]/g,"")}))} autoComplete="cc-exp" inputMode="numeric" />{cardErrors.expiry&&<div className="form-error">{cardErrors.expiry}</div>}</div>
                <div className="form-group"><label className="form-label" htmlFor="card-cvc">CVC</label><input id="card-cvc" name="cardCvc" className="form-input" placeholder="123" maxLength={4} type="password" value={newCard.cvc} onChange={e=>setNewCard(p=>({...p,cvc:e.target.value.replace(/\D/g,"")}))} autoComplete="cc-csc" inputMode="numeric" />{cardErrors.cvc&&<div className="form-error">{cardErrors.cvc}</div>}</div>
              </div>
            </div>
...
export default function App() {
  ...
  // Fix: Inject favicon to resolve 404 /favicon.ico browser error
  useEffect(() => {
    const svgFavicon = `data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='8' fill='%236366f1'/><text x='16' y='23' text-anchor='middle' font-size='19' font-family='Arial,sans-serif' fill='white'>✦</text></svg>`;
    ...
    // Also set page title for polish
